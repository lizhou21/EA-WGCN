...
def main():
    ...
    if args.lower:      # 将token中的大写全转为小写
        train_tokens, dev_tokens, test_tokens = [[t.lower() for t in tokens] for tokens in\
                (train_tokens, dev_tokens, test_tokens)]

    # load glove
    print("loading glove...")    # 只用训练集生成词表
    glove_vocab = vocab.load_glove_vocab(wv_file, wv_dim)    # 加载预训练glove词向量
...
def build_vocab(tokens, glove_vocab, min_freq):
    """ build vocab from tokens and glove words. 建立词典"""
...
def entity_masks():
    """ Get all entity mask tokens as a list. 所有实体对应的mask token列表，每种类型的实体用特殊的符号代替"""
    masks = []
